Running Atlas in Docker | Atlas Guides
...
`latest`, `latest-distroless`

Bare bone image containing only Atlas

Alpine

`latest-alpine`

Alpine based image, with basic shell (/bin/sh)

## Common Issues[​](#common-issues "Direct link to Common Issues")


### `Use 'atlas login' to access this feature`[​](#use-atlas-login-to-access-this-feature "Direct link to use-atlas-login-to-access-this-feature")
...
4.  Copy the code provided by Atlas Cloud: ![Screenshot of the Atlas Cloud authentication page, showing the auth code to be copied.](/assets/images/copy-this-code-44ed37cdfc8aec5ce119b9da59111981.png)
...
7.  You can now use Atlas features that require authentication. Use the `-v ~/.atlas:/root/.atlas` flag to persist your login credentials across Docker runs. For example:
    ...
    ```codeBlockLines_AdAo
     docker run --rm -it \   -v ~/.atlas:/root/.atlas \   arigaio/atlas:latest-alpine schema inspect --url "<url>"
    ```

### `"docker": executable file not found in $PATH`[​](#docker-executable-file-not-found-in-path "Direct link to docker-executable-file-not-found-in-path")


Atlas heavily relies on the presence of a [Dev Database](/concepts/dev-database) for various calculations and schema normalization. To use a Dev Database, users provide Atlas with the URL to connect to an empty database of the type they wish to operate on.
...
#### Workaround: Spin up a local database container and use it[​](#workaround-spin-up-a-local-database-container-and-use-it "Direct link to Workaround: Spin up a local database container and use it")
...
```codeBlockLines_AdAo
docker run --rm --network db-network \  -v $PWD:/data \  arigaio/atlas migrate diff \  --to file:///data/schema.hcl \  --dir file:///data/migrations \  --dev-url "postgres://postgres:mysecretpassword@pg-dev:5432/postgres?sslmode=disable"
```
...
*   [Common Issues](#common-issues)
    *   [`Use 'atlas login' to access this feature`](#use-atlas-login-to-access-this-feature)
    *   [`"docker": executable file not found in $PATH`](#docker-executable-file-not-found-in-path)