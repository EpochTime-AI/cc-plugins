Deploying Schema Migrations to Database-per-Tenant Architecture | Atlas Guides
...
### Our config file[​](#our-config-file "Direct link to Our config file")
...
### An initial migration[​](#an-initial-migration "Direct link to An initial migration")
...
## Deploying the migrations[​](#deploying-the-migrations "Direct link to Deploying the migrations")
...
```codeBlockLines_AdAo
Migrating to version 20240721101205 (1 migrations in total):  -- migrating version 20240721101205    -> CREATE TABLE users (         id INTEGER PRIMARY KEY,         name TEXT NOT NULL       );  -- ok (345.458µs)  -------------------------  -- 3.400333ms  -- 1 migration  -- 1 sql statementMigrating to version 20240721101205 (1 migrations in total):  -- migrating version 20240721101205    -> CREATE TABLE users (         id INTEGER PRIMARY KEY,         name TEXT NOT NULL       );  -- ok (266.375µs)  -------------------------  -- 905.875µs  -- 1 migration  -- 1 sql statement
```
...
### Verifying our migrations were applied[​](#verifying-our-migrations-were-applied "Direct link to Verifying our migrations were applied")
...
Atlas prints:
```codeBlockLines_AdAo
Migration Status: OK  -- Current Version: 20240721101205  -- Next Version:    Already at latest version  -- Executed Files:  1  -- Pending Files:   0
```
As expected, the `tenant_1` database is up-to-date with the latest migration.

### Checking for Drift[​](#checking-for-drift "Direct link to Checking for Drift")
...
```codeBlockLines_AdAo
atlas schema diff \  --dev-url 'sqlite://?mode=memory' \  --from file://migrations \  --to sqlite://tenant_1.db \  --exclude 'atlas_schema_revisions'
```
Atlas will output:
```codeBlockLines_AdAo
...
*   [Setting up](#setting-up)
    ...
    *   [An initial migration](#an-initial-migration)
*   [Deploying the migrations](#deploying-the-migrations)
    *   [Verifying our migrations were applied](#verifying-our-migrations-were-applied)
