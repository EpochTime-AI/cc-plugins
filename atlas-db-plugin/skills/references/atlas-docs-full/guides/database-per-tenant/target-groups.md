Defining Target Groups for Multi-Tenant Deployments | Atlas Guides
...
Target groups are defined in the project's [`atlas.hcl`](/atlas-schema/projects) file and are later used by the Atlas CLI during the deployment process to determine which databases to deploy to.

Let's review some examples of how to define target groups in Atlas.

## `env` blocks and `for_each` meta-arguments[​](#env-blocks-and-for_each-meta-arguments "Direct link to env-blocks-and-for_each-meta-arguments")
...
## Dynamically Computing URLs[​](#dynamically-computing-urls "Direct link to Dynamically Computing URLs")
...
## Loading data from local JSON files[​](#loading-data-from-local-json-files "Direct link to Loading data from local JSON files")
...
atlas.hcl
```codeBlockLines_AdAo
locals {  f = file("tenants.json")  decoded = jsondecode(local.f)  tenants = local.decoded.tenants}env "tenants" {  for_each = toset(local.tenants)  url = urlsetpath("postgres://root:pass@localhost:5432", each.value)}
```
...
atlas.hcl
```codeBlockLines_AdAo
env "tenants" {  for_each = toset(local.target_tenants)  url = urlsetpath("postgres://root:pass@localhost:5432", each.value)}
```
Let's review the code snippet above:
...
## Loading Data from an API Endpoint[​](#loading-data-from-an-api-endpoint "Direct link to Loading Data from an API Endpoint")
...
```codeBlockLines_AdAo
{  "databases": [    "acme_corp",    "widget_inc",    "wayne_enterprises",    "stark_industries"  ]}
```
...
```codeBlockLines_AdAo
var "group_id" {    type = string}data "runtimevar" "tenants" {  url = "http://tenant-svc/api/target-group/${var.group_id}"}locals {  decoded = jsondecode(data.runtimevar.tenants)  tenants = local.decoded.databases}env "tenants" {  for_each = toset(local.tenants)  url = urlsetpath("postgres://root:pass@localhost:5432", each.value)}
```
...
## Loading data from a Database Query[​](#loading-data-from-a-database-query "Direct link to Loading data from a Database Query")
...
```codeBlockLines_AdAo
var "url" {  type = string}locals {    pattern = "tenant_%"}data "sql" "tenants" {  url = var.url  query = <<EOSSELECT `schema_name`  FROM `information_schema`.`schemata`  WHERE `schema_name` LIKE ?EOS  args = [local.pattern]}env "prod" {  for_each = toset(data.sql.tenants.values)  url      = urlsetpath(var.url, each.value)}
```
...
### Fetching Tenant Metadata[​](#fetching-tenant-metadata "Direct link to Fetching Tenant Metadata")


For more advanced use cases, such as [deployment rollout strategies](/guides/database-per-tenant/rollout), you may need to fetch additional metadata about each tenant (e.g., tier, region, or status). The `sql` data source can return multiple columns per row, which are accessible as a map:
```codeBlockLines_AdAo
data "sql" "tenants" {  url   = var.management_url  query = <<EOSSELECT  `org`.`name`,  `org`.`schema`,  `plan`.`tier`FROM `management`.`organizations` AS `org`LEFT JOIN `management`.`plans` AS `plan`  ON `org`.`plan_id` = `plan`.`id`EOS}env "prod" {  for_each = toset(data.sql.tenants.values)  url      = urlsetpath(var.url, each.value.schema)  // each.value.name and each.value.tier are also available}
```
This metadata can be used to configure [staged rollouts](/guides/database-per-tenant/rollout) based on tenant attributes like tier, region, or priority.

## Incorporating Sensitive Data[​](#incorporating-sensitive-data "Direct link to Incorporating Sensitive Data")
...
```codeBlockLines_AdAo
var "db_instance_url" {  type = string}var "db_user" {  type = string}data "runtimevar" "pass" {  url = "awssecretsmanager://db-pass-demo?region=us-east-1"}locals {  db_with_pass = urluserinfo(var.db_instance_url, var.db_user, data.runtimevar.pass.value)  tenants = ["acme_corp", "widget_inc", "wayne_enterprises", "stark_industries"]}env "tenants" {  for_each = toset(local.tenants)  url = urlsetpath(local.db_with_pass, each.value)}
```
...
```codeBlockLines_AdAo
urluserinfo("postgres://localhost:5432", "root", "p455w0rd")#  ↳ Evaluates to "postgres://root:p455w0rd@localhost:5432"
```
...
*   [`env` blocks and `for_each` meta-arguments](#env-blocks-and-for_each-meta-arguments)
*   [Dynamically Computing URLs](#dynamically-computing-urls)
*   [Loading data from local JSON files](#loading-data-from-local-json-files)
*   [Loading Data from an API Endpoint](#loading-data-from-an-api-endpoint)
*   [Loading data from a Database Query](#loading-data-from-a-database-query)
    *   [Fetching Tenant Metadata](#fetching-tenant-metadata)
*   [Incorporating Sensitive Data](#incorporating-sensitive-data)
