Testing Database Triggers | Atlas Guides
...
schema.test.hcl
```codeBlockLines_AdAo
test "schema" "trigger" {  # Seed data  exec {    sql = "INSERT INTO products (id, price) VALUES (1, 15.00), (2, 22.99), (3, 13.50);"  }  # Verify products_audit table empty  exec {    sql = "SELECT COUNT(*) FROM products_audit"    output = "0"  }  for_each = [    {product_id: "1", new_price: "19.99"},    {product_id: "2", new_price: "20"}  ]  exec {    sql = "UPDATE products SET price = ${each.value.new_price} WHERE id = ${each.value.product_id};"  }  exec {    sql = "SELECT new_price FROM products_audit where product_id = ${each.value.product_id}"    output = each.value.new_price  }  log {    message = "Testing for ID:  ${each.value.product_id} - New Price: ${each.value.new_price}"  }}
```
Run the test by running:
```codeBlockLines_AdAo
atlas schema test --env dev
...
Test Output
```codeBlockLines_AdAo
-- PASS: trigger/1 (4ms)    schema.test.hcl:50: Testing for ID:  1 - New Price: 19.99-- PASS: trigger/2 (1ms)    schema.test.hcl:50: Testing for ID:  2 - New Price: 20PASS
```
*   [Triggers](#triggers)
