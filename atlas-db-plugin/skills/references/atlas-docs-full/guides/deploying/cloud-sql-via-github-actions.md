Deploying schema migrations to Google Cloud SQL using Atlas and GitHub Actions | Atlas Guides
...
## In this article[​](#in-this-article "Direct link to In this article")


*   [Overview](#overview)
*   [What is Cloud SQL?](#what-is-cloud-sql)
*   [What is Cloud SQL Auth Proxy?](#what-is-cloud-sql-auth-proxy)
*   [What is GitHub Actions?](#what-is-github-actions%5D)
...
## What is Cloud SQL?[​](#what-is-cloud-sql "Direct link to What is Cloud SQL?")
...
## What is Cloud SQL Auth Proxy?[​](#what-is-cloud-sql-auth-proxy "Direct link to What is Cloud SQL Auth Proxy?")
...
## What is GitHub Actions?[​](#what-is-github-actions "Direct link to What is GitHub Actions?")
...
## Deploying Schema Migrations to Cloud SQL[​](#deploying-schema-migrations-to-cloud-sql "Direct link to Deploying Schema Migrations to Cloud SQL")
...
#### 1—Authenticate to Google Cloud[​](#1authenticate-to-google-cloud "Direct link to 1—Authenticate to Google Cloud")
...
```
Replace **\[PROJECT\_NAME\]** with the name of your project, and **\[SERVICE\_ACCOUNT\_EMAIL\]** with the email address of the service account you want to grant access to.

5.  Create a new workload identity pool:
```codeBlockLines_AdAo
$ gcloud iam workload-identity-pools create "my-pool" \  --project="${PROJECT_ID}" \  --location="global" \  --display-name="My pool"
```
...
```codeBlockLines_AdAo
$ export WORKLOAD_IDENTITY_POOL_ID="..." # value from above# This should look like:##   projects/123456789/locations/global/workloadIdentityPools/my-pool#
```
7.  Create a Workload Identity Provider in that pool:
```codeBlockLines_AdAo
$ gcloud iam workload-identity-pools providers create-oidc "my-provider" \  --project="${PROJECT_ID}" \  --location="global" \  --workload-identity-pool="my-pool" \  --display-name="GitHub provider" \ --attribute-mapping="google.subject=assertion.sub,attribute.actor=assertion.actor,attribute.repository=assertion.repository" \  --issuer-uri="https://token.actions.githubusercontent.com"
```
...
```
Use this value as the **workload\_identity\_provider** value in your GitHub Actions YAML.
...
#### 2—Retrieve your Instance Connection Name[​](#2retrieve-your-instance-connection-name "Direct link to 2—Retrieve your Instance Connection Name")
...
#### 3—Store your Password in GitHub Secrets[​](#3store-your-password-in-github-secrets "Direct link to 3—Store your Password in GitHub Secrets")
...
5.  Enter **"DB\_PASSWORD"** in the **"Name"** field.
...
#### 4—Setup GitHub Actions[​](#4setup-github-actions "Direct link to 4—Setup GitHub Actions")
...
#### 5—Execute your GitHub Actions Workflow[​](#5execute-your-github-actions-workflow "Direct link to 5—Execute your GitHub Actions Workflow")


To execute this workflow once you commit to the main branch, follow these steps:

1.  Create a new file named **atlas\_migrate\_db.yml** in the **.github/workflows/** directory of your repository.
2.  Add the code block we've just discussed to the **atlas\_migrate\_db.yml** file.
3.  Commit the **atlas\_migrate\_db.yml** file to your repository's **main** branch.
