Running Schema Migrations on AWS ECS/Fargate using Atlas | Atlas Guides
...
[AWS Elastic Container Service (ECS)](https://aws.amazon.com/ecs/) is a popular way to deploy containerized applications to AWS. ECS is a managed service that allows you to run containers on a cluster of EC2 instances, or on AWS Fargate, a serverless compute engine for containers.
...
1.  [A service running on ECS/Fargate](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ECS_AWSCLI_Fargate.html) defining your application.
...
## Storing database credentials in Secrets Manager[​](#storing-database-credentials-in-secrets-manager "Direct link to Storing database credentials in Secrets Manager")
...
## Reading secrets during deployment[​](#reading-secrets-during-deployment "Direct link to Reading secrets during deployment")


To read our secret value during deployment we can use the [`runtimevar`](https://atlasgo.io/atlas-schema/projects#data-source-runtimevar) data source. To use this, create a project file named `atlas.hcl`:
```codeBlockLines_AdAo
...
Next, create a Dockerfile that will include your migration directory and project file. This is a variation of the baseline example we introduced in the ["Creating container images for migrations"](/guides/deploying/image) guide:
```codeBlockLines_AdAo
FROM arigaio/atlas:latestCOPY migrations /migrationsCOPY atlas.hcl .
```
This image should be built and pushed to ECR (or another container registry) as part of your CI process.

### Running migrations before the application starts[​](#running-migrations-before-the-application-starts "Direct link to Running migrations before the application starts")
...
```codeBlockLines_AdAo
{   "family":"fargate-demo-task-dev",   "taskRoleArn":"arn:aws:iam::<account id>:role/fargate-demo-ecsTaskRole",   "executionRoleArn":"arn:aws:iam::<account id>:role/fargate-demo-ecsTaskExecutionRole",   "networkMode":"awsvpc",   "requiresCompatibilities":[      "FARGATE"   ],   "cpu":"256",   "memory":"512",   "containerDefinitions":[      {         "name":"atlas",         "image":"<account id>.dkr.ecr.us-east-2.amazonaws.com/fargate-demo:v5",         "essential":false,         "command":[            "migrate",            "apply",            "--env",            "deploy"         ]      },      {         "name":"fargate-demo-container-dev",         "image":"nginx:latest",         "portMappings":[            {               "containerPort":80,               "hostPort":80,               "protocol":"tcp"            }         ],         "essential":true,         "dependsOn":[            {               "containerName":"atlas",               "condition":"SUCCESS"            }         ],      }   ]}
```
...
*   [Storing database credentials in Secrets Manager](#storing-database-credentials-in-secrets-manager)
*   [Reading secrets during deployment](#reading-secrets-during-deployment)
    *   [Running migrations before the application starts](#running-migrations-before-the-application-starts)